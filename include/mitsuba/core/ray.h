...
/// Return a string representation of the rayDifferential
template <typename Point, typename Spectrum>
std::ostream &operator<<(std::ostream &os, const RayDifferential<Point, Spectrum> &r) {
    os << "RayDifferential" << type_suffix<Point>() << "[" << std::endl
       << "  o = " << string::indent(r.o, 6) << "," << std::endl
       << "  o_x = " << string::indent(r.o_x, 6) << "," << std::endl
       << "  o_y = " << string::indent(r.o_y, 6) << "," << std::endl
       << "  d = " << string::indent(r.d, 6) << "," << std::endl
       << "  d_x = " << string::indent(r.d_x, 6) << "," << std::endl
       << "  d_y = " << string::indent(r.d_y, 6) << "," << std::endl
       << "  mint = " << r.mint << "," << std::endl
       << "  maxt = " << r.maxt << "," << std::endl
       << "  time = " << r.time << "," << std::endl;
    if (r.wavelengths.size() > 0)
