...
public:
    MTS_IMPORT_TYPES(BSDF, Emitter, Film, Sampler, Shape, Sensor, Integrator, Medium, MediumPtr, ShapePtr)
    ...
    /**
     * \brief Direct illumination thought glass panel sampling routine
     *
     * Given an arbitrary reference point in the scene, this method samples a
     * direction from the reference point to towards an emitter, with a double refractive interface in between.
     *
     *
     * \param ref
     *    A reference point somewhere within the scene
...
protected:
    ...
    std::vector<ref<Shape>> m_caustic_casters_double_refraction;
