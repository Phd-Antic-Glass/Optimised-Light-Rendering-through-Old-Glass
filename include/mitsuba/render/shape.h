 ...
 */
template <typename Float, typename Spectrum> class MTS_EXPORT_RENDER Shape : public Object {
public:
     ...
     */
    virtual PositionSample3f sample_position(Float time, const Point2f &sample, Mask active = true) const;
     ...
     */
    virtual DirectionSample3f sample_direction(const Interaction3f &it, const Point2f &sample, Mask active = true) const;
     ...
     */
    virtual Float pdf_direction(const Interaction3f &it, const DirectionSample3f &ds, Mask active = true) const;
     ...
     */
    virtual std::pair<Mask, Float> ray_intersect(const Ray3f &ray, Float *cache, Mask active = true) const;
     ...
     */
    virtual void fill_surface_interaction(const Ray3f &ray, const Float *cache, SurfaceInteraction3f &si, Mask active = true) const;
     ...
     */
    virtual std::pair<Vector3f, Vector3f> normal_derivative(const SurfaceInteraction3f &si, bool shading_frame = true,
    ...
    virtual Array<Float, 6> eval_curvature(Point2f p, Mask active = true, bool full=true) const;
    virtual SurfaceInteraction3f eval_surfaceInteraction_from_uv(Point2f p, Mask active, SurfaceInteraction3f &si_) const;
    ...
    /// Is this shape a (m_double_refraction) caustic caster for AG guided path?
    bool is_caustic_caster_double_refraction() const { return m_double_refraction; }

    /// Does the surface of this shape mark a medium transition?
    bool is_medium_transition() const { return m_interior_medium.get() != nullptr || m_exterior_medium.get() != nullptr; }
    ...
    ScalarTransform4f get_local_transform() const { return m_to_object; }
    ScalarTransform4f get_world_transform() const { return m_to_world; }
...
protected:
    ...
    bool m_double_refraction     = false;
    ref<BSDF> m_bsdf;
...
ENOKI_CALL_SUPPORT_GETTER_TYPE(interior_medium, m_interior_medium, const typename Class::Medium *)
ENOKI_CALL_SUPPORT_GETTER_TYPE(exterior_medium, m_exterior_medium, const typename Class::Medium *)
auto is_emitter() const { return neq(emitter(), nullptr); }
auto is_sensor() const { return neq(sensor(), nullptr); }
auto is_medium_transition() const { return neq(interior_medium(), nullptr) || neq(exterior_medium(), nullptr); }
