...
struct ManifoldVertex {
    ...
    ManifoldVertex(const SurfaceInteraction3f &si,Sampler *sampler, bool forceIOR_eq1, Float smoothing=0.f)
        ...
        if(real(si.bsdf()->ior(si)) != 1.0f && forceIOR_eq1)
        {
            Float cos = dot(si.n,si.wi);
            Float n1 = 1.0f;
            Float n2 = 1.0f;
            if(cos >=0)
            {
    ...
    std::string to_string() const {
        ...
        oss << "ManifoldVertex[" << std::endl
            ...
            << "  eta = " << eta << "," << std::endl
            << "  uv = " << uv << std::endl
            << "  fixed_direction = " << fixed_direction << std::endl
            << "]";
        return oss.str();
    }
};
...
struct SpecularManifold {
    ...
    emitter_interaction_to_vertex(const Scene *scene,
                                  ...
                                  const Point3f &p,
                                  Float time,
                                  const Wavelength &wavelengths,Sampler *sampler,bool activated) {
        if (ei.is_area()) {
            // Area emitters
            ...
            ManifoldVertex vy(si_y,sampler,activated);
            vy.make_orthonormal();
