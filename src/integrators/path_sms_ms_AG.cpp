...
class AGSMSPathIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    using SpecularManifoldSingleScatter = SpecularManifoldSingleScatter_AG<Float, Spectrum>;
...
//    static inline ThreadLocal<SpecularManifoldSingleScatter_AG> tl_manifold_ss{};
    static inline ThreadLocal<MNEEHelper> tl_mnee{};

public:
    AGSMSPathIntegrator(const Properties &props) : Base(props) {
        ...
        // bool multi_piece_mode = props.bool_("multi_piece_mode", false); // placeholder


        m_biased_mnee                  = props.bool_("biased_mnee", false);

        m_sms_ss_config = SMSConfig();
        m_sms_ss_config.biased                 = props.bool_("biased", false);
        m_sms_ss_config.twostage               = props.bool_("twostage", false);
        m_sms_ss_config.halfvector_constraints = props.bool_("halfvector_constraints", false);
        m_sms_ss_config.mnee_init              = props.bool_("mnee_init", false);
        m_sms_ss_config.step_scale             = props.float_("step_scale", 1.f);
        m_sms_ss_config.max_iterations         = props.int_("max_iterations", 20);
        m_sms_ss_config.solver_threshold       = props.float_("solver_threshold", 1e-5f);
        m_sms_ss_config.uniqueness_threshold   = props.float_("uniqueness_threshold", 1e-4f);
        m_sms_ss_config.max_trials             = props.int_("max_trials", -1);
        m_sms_ss_config.bounces                = props.int_("bounces", 1);
 ...
 //       SpecularManifoldSingleScatter_AG::print_statistics();
        SpecularManifoldMultiScatter_AG::print_statistics();
        return result;
    }
        ...
        if constexpr (is_array_v<Float>) {
            ...
            for (int depth = 1;; ++depth) {
                ...
                if (uint32_t(depth) >= uint32_t(m_max_depth))
                    break;
                // printf("%d / %d \n",depth+ m_sms_config.bounces,m_max_depth);
			...
			SpecularManifoldSingleScatter mf(scene, m_sms_ss_config);
                        result += throughput * mf.specular_manifold_sampling(si, sampler);
                     }

                    if (m_max_depth < 0 || depth + m_sms_config.bounces < m_max_depth) {
                ...
                if (emitter) {
                    ...
                    if (!on_caustic_caster || specular_camera_path) {
                        ...
                        /* These are the light paths that can be sampled with SMS.
                           ...
                           passed to the SpecularManifoldMultiScatter_AG datastructure
                           somehow. */
                        ...
                        bool success = mf_ms.sample_path(specular_shape, mnee.si_endpoint, ei, sampler, true);
                        if (success) {
                            auto current_path = mf_ms.current_path();
    ...
    std::string to_string() const override {
        return tfm::format("AGSMSPathIntegrator[\n"
            "  max_depth = %i,\n"
            "  rr_depth = %i\n"
...
protected:
    ...
    SMSConfig m_sms_ss_config;
};

MTS_IMPLEMENT_CLASS_VARIANT(AGSMSPathIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(AGSMSPathIntegrator, "Multi-Bounce SMS Path Tracer integrator");
