...
template <typename Float, typename Spectrum> class path_fnee : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    using SpecularManifoldMultiScatter_AG  = SpecularManifoldMultiScatter_AG<Float, Spectrum>;
    using SpecularManifoldSingleScatter_AG = SpecularManifoldSingleScatter_AG<Float, Spectrum>;
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler, const RayDifferential3f &ray_, const Medium * /* medium */,
                                     Float * /* aovs */, Mask active) const override {
        MTS_MASKED_FUNCTION(ProfilerPhase::SamplingIntegratorSample, active);
        // auto &mf_ms       = (SpecularManifoldMultiScatter_AG &) tl_manifold;
        auto &NEE_sampler = (fermatNEE &) tl_fermaNEE;
        // mf_ms.init(scene, m_sms_config);
        NEE_sampler.init(scene, sampler, m_sms_config.uniqueness_threshold, m_sms_config.max_trials, alpha, beta, crop_caustic,
                         m_sms_config);
        ...
        for (int depth = 1;; ++depth) {
            ...
            // TODO have two integrator: <launch solver for shadow only> / <global>
            bool doubleRefraction    = false;
            bool on_caustic_receiver = si.shape->is_caustic_receiver();
            bool on_caustic_caster   = si.shape->is_caustic_caster_single_scatter() || si.shape->is_caustic_caster_multi_scatter() ||
...
//                    if (sample_heighfield_success) {
                        ...
                        // std::cout << H1 << H2 << std::endl << std::endl;

                        // TODO gerer direction sample
                        // typename fermatNEE::Emitter_vertex vy = NEE_sampler.sample_emitter(si, active);
                        typename SpecularManifold::EmitterInteraction vy = SpecularManifold::sample_emitter_interaction(si, scene->caustic_emitters_multi_scatter(), sampler);
                      

                    /* +--------------------+
                    // |// double refraction|
                    // +--------------------+ */
		    Float invpdf_h_sampling = 1;
                    //bool sample_heighfield_success = NEE_sampler.sample_heightfield_pair(&H1, &H2, &invpdf_h_sampling,active);
                    bool sample_heighfield_success = scene->test_double_refraction_NEE(ctx, si, Vector3f(0), vy.p, &H1, &H2,active, sampler->next_2d(), sampler->next_1d());
                    if(sample_heighfield_success){
			...
			Float NEE_invpdf                      = 1.0f;
                        ...
                        // std::cout<< Vector3f(1,1,1) << "|" << H1->get_local_transform()*(Vector3f(1,1,1)) <<std::endl;
                        ...
                        Float inv_partition_size                                = H1->eval_attribute_1("inv_partition_size", si);
                        Float inv_partition_area                                = H1->eval_attribute_1("inv_partition_area", si);
                        typename fermatNEE::L_data_float fermat_connection_data = { H1,
                                                                                    ...
                                                                                    partition_size,
                                                                                    inv_partition_size,
                                                                                    inv_partition_area }; // needs local coordinates

                        Vector3f fermat_connection_direction;
                        bool FNEE_success = false;

                        FNEE_success = NEE_sampler.fermat_connection(&fermat_connection_data, &fermat_connection_direction);

                        if (FNEE_success) {
			    result[active] += Spectrum(1);

                            r.d = H1->get_world_transform().transform_affine(fermat_connection_direction);
                            r.d = (r.d);
                            r.update();
                            Vector3f wo_bsdf            = (si.to_local(r.d));
                            Spectrum bsdf_shading_point = si.bsdf()->eval(ctx, si, wo_bsdf);
                            //bsdf_shading_point          = si.to_world_mueller(bsdf_shading_point, -wo_bsdf, si.wi);

                            // OpenGL_viewer_client<Float, Spectrum>::OpenGL_command_drawline(r.o, r.o + r.d * 2, Vector3f(1,1,1));
                            NEE_specularOutput = NEE_sampler.compute_ray_contribution(si, ctx, r, vy, 1.0f, 1.54f, H1, H2, active);
                            if (NEE_specularOutput != Spectrum(0)) {

                                NEE_invpdf = NEE_sampler.eval_invPDF(&fermat_connection_data, fermat_connection_direction);

                                result[active] +=
                                    throughput * bsdf_shading_point * vy.weight * NEE_specularOutput * NEE_invpdf * invpdf_h_sampling; // *NEE_invpdf
                            }
                        }
                    }
                    ...
                    // sample_heighfield_success = NEE_sampler.sample_heightfield(&H1, &invPdf_H_sampling, active);
                    // if (sample_heighfield_success) {
                    //     // TODO gerer direction sample
                    //     typename SpecularManifold::EmitterInteraction vy = SpecularManifold::sample_emitter_interaction(si, scene->caustic_emitters_multi_scatter(), sampler);
                    //     Float NEE_invpdf                      = 1.0f;
                    ...
                    //     Vector3f wo_r = normalize(vy.p - si.p);
                    //     // Ray3f r(si.p, wo_r, math::RayEpsilon<Float> * (1.f + hmax(abs(si.p))),
                    //     //                     math::Infinity<Float>, si.time, si.wavelengths);
                    //     Ray3f r;
                    //     r.o    = si.p;
                    //     r.mint = math::RayEpsilon<Float> * (1.f + hmax(abs(si.p)));
                    //     r.time = si.time;
                    ...
                    //     FNEE_success = NEE_sampler.fermat_connection_reflection(&fermat_connection_data, &fermat_connection_direction);
                    //     if (FNEE_success) {
                    ...
                    //         NEE_specularOutput =
                    //             NEE_sampler.compute_ray_contribution_reflection(si, ctx, r, vy, 1.0f, 1.54f, H1, nullptr, active);

                    //         if (NEE_specularOutput != Spectrum(0)) {

                    //             NEE_invpdf     = NEE_sampler.eval_invPDF_reflection(&fermat_connection_data,
                    //             fermat_connection_direction); Vector3f wo    = (si.to_local(r.d)); Float bsdf_pdf = bsdf->pdf(ctx, si,
                    //             wo, active); Float mis      = mis_weight(vy.pdf, bsdf_pdf);
		  ...
		  typename SpecularManifold::EmitterInteraction vy = SpecularManifold::sample_emitter_interaction(si, scene->caustic_emitters_multi_scatter(), sampler);
                    ...
                    // |// double refraction|
                    // +--------------------+ */
                    Float invPdf_heightfield_pair = 0;
		    // bool sample_heighfield_success = NEE_sampler.sample_heightfield_pair(&H1, &H2, &invPdf_heightfield_pair, active);
                    bool sample_heighfield_success = scene->test_double_refraction_NEE(ctx, si, Vector3f(0), vy.p, &H1, &H2,active, sampler->next_2d(), sampler->next_1d());
		    ...
		    Point3f origin_H1              = H1->get_world_transform().transform_affine(Point3f(0.f));
                    ...
                    if (squared_norm(O_local_H1) > squared_norm(O_local_H1 - origin_H2_local_H1)) {
                        ...
                        FNEE_success = NEE_sampler.SMS_connection(si, vy, &fermat_connection_data, &fermat_connection_direction, sampler);

                        if (FNEE_success) {
                            result[active] += Spectrum(100.0f);
                           ...
                           // bsdf_shading_point          = si.to_world_mueller(bsdf_shading_point, -wo_bsdf, si.wi);
                            ...
                            if (NEE_specularOutput != Spectrum(0)) {
                                ...
                                NEE_invpdf = NEE_sampler.SMS_eval_invPDF(si, vy, &fermat_connection_data, fermat_connection_direction, sampler);

                                //result[active] +=
                                  //  throughput * bsdf_shading_point * vy.weight * NEE_specularOutput * NEE_invpdf * sample_heightfield_invpdf; // *NEE_invpdf
                            }
                        }
                    }
                    ...
                    //             NEE_invpdf     = NEE_sampler.eval_invPDF_reflection(&fermat_connection_data,
                    //             fermat_connection_direction); Vector3f wo    = (si.to_local(r.d)); Float bsdf_pdf = bsdf->pdf(ctx, si,
                    //             wo, active_e); Float mis      = mis_weight(vy.pdf, bsdf_pdf);

                    //             result[active_e] += throughput * bsdf_shading_point * vy.weight * NEE_specularOutput * NEE_invpdf *
                    //             invPdf_H_sampling; // *NEE_invpdf
                    //         }
                    //     }
                    // }
            ...
            /* As usual, emitter sampling only makes sense on Smooth BSDFs
               ...
               useful. */
            if (has_flag(bsdf->flags(), BSDFFlags::Smooth) && !on_caustic_caster) {
                // if (NEE_specularOutput == Spectrum(0.f)) {
                auto [ds, emitter_val] = scene->sample_emitter_direction(si, sampler->next_2d(active_e), true, active_e);
            ...
            if (emitter) {
                ...
                if (!on_caustic_caster || specular_camera_path) {
                    ...
                    ds.object         = emitter;
                    Float emitter_pdf = select(!has_flag(bs.sampled_type, BSDFFlags::Delta), scene->pdf_emitter_direction(si, ds), 0.f);
