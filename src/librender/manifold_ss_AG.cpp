...
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter_AG<Float, Spectrum>::stats_solver_failed(0);
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter_AG<Float, Spectrum>::stats_solver_succeeded(0);
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter_AG<Float, Spectrum>::stats_bernoulli_trial_calls(0);
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter_AG<Float, Spectrum>::stats_bernoulli_trial_iterations(0);
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter_AG<Float, Spectrum>::stats_bernoulli_trial_iterations_max(0);

MTS_VARIANT SpecularManifoldSingleScatter_AG<Float, Spectrum>::SpecularManifoldSingleScatter_AG(
...
SpecularManifoldSingleScatter_AG<Float, Spectrum>::specular_manifold_sampling(const SurfaceInteraction3f &si,
    ...
    for (size_t shape_idx = 0; shape_idx < shapes.size(); ++shape_idx) {
        ...
        if (!m_config.biased) {
            ...
            Spectrum specular_val = evaluate_path_contribution(si, ei, si_final,sampler);
...
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Mask,
                       typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::SurfaceInteraction3f,
                       typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::SurfaceInteraction3f>
SpecularManifoldSingleScatter_AG<Float, Spectrum>::sample_path(const ShapePtr shape,
    ...
    ManifoldVertex vtx_init(si_init,true, m_config.twostage ? 1.f : 0.f);
    ...
    if (m_config.twostage) {
        ...
        auto [success_smooth, si_smooth] = newton_solver(si, vtx_init, ei, sampler, lean_normal_local, 1.f);
        if (success_smooth && si_smooth.is_valid()) {
            vtx_init = ManifoldVertex(si_smooth,true, 0.f);
        } else {
            SurfaceInteraction3f si_dummy;
            return std::make_tuple(false, si_dummy, si_dummy);
        }
    ...
    auto [success, si_final] = newton_solver(si, vtx_init, ei,sampler, n_offset, 0.f);
...
MTS_VARIANT Spectrum
SpecularManifoldSingleScatter_AG<Float, Spectrum>::evaluate_path_contribution(const SurfaceInteraction3f &si,
                                                                           const EmitterInteraction &ei_,
                                                                           const SurfaceInteraction3f &si_final,Sampler *sampler) const {
    // Specular point to ManifoldVertex
    ManifoldVertex vtx(si_final,sampler,true);
    ...
    ManifoldVertex vx = ManifoldVertex(si,sampler,true);
    vx.make_orthonormal();

    Spectrum path_throughput(1.f);
    path_throughput *= specular_reflectance(si_final, normalize(ei.p - si_final.p),vtx);
    path_throughput *= geometric_term(vx, vtx, vy);
    path_throughput *= ei.weight;
    return path_throughput;
}

MTS_VARIANT std::pair<typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Mask,
                      typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::SurfaceInteraction3f>
SpecularManifoldSingleScatter_AG<Float, Spectrum>::newton_solver(const SurfaceInteraction3f &si,
                                                              const ManifoldVertex &vtx_init,
                                                              const EmitterInteraction &ei,Sampler *sampler,
                                                              const Vector3f &n_offset,
    ...
    while (iterations < m_config.max_iterations) {
        ...
        vtx = ManifoldVertex(si_current,true, smoothing);
    ...
    if ((vtx.eta == 1.f && !reflection) || (vtx.eta != 1.f && !refraction)) {
        return { false, si_current };
    }
...
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Mask,
                       typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Vector2f,
                       typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Vector2f>
SpecularManifoldSingleScatter_AG<Float, Spectrum>::compute_step_halfvector(const Point3f &v0p,
...
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Mask,
                       typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Vector2f,
                       typename SpecularManifoldSingleScatter_AG<Float, Spectrum>::Vector2f>
SpecularManifoldSingleScatter_AG<Float, Spectrum>::compute_step_anglediff(const Point3f &v0p,
                                                                       const ManifoldVertex &v1,
                                                                       const EmitterInteraction &v2,
                                                                       const Vector3f &n_offset) const {
...
MTS_VARIANT Spectrum
SpecularManifoldSingleScatter_AG<Float, Spectrum>::specular_reflectance(const SurfaceInteraction3f &si,
                                                                     const Vector3f &wo,ManifoldVertex vtx) const {
    if (!si.is_valid()) return 0.f;
    const BSDFPtr bsdf = si.shape->bsdf();
    if (!bsdf) return 0.f;

    Complex<Spectrum> ior = si.bsdf()->ior(si);
    Mask reflection = any(eq(1.0f, vtx.eta));
    // Mask reflection = any(neq(0.f, imag(ior)));
    ...
    if (bsdf->roughness() > 0.f) {
        ...
        if (reflection) {
            if (all(eq(imag(ior), 0.f))) {
                ...
                // auto [F, unused_0, unused_1, unused_2] = fresnel(Spectrum(cos_theta), real(ior));
                // bsdf_val = F;
            }
        } else {
            // printf("REFRACTION\n");
...
MTS_VARIANT Float
SpecularManifoldSingleScatter_AG<Float, Spectrum>::geometric_term(const ManifoldVertex &v0,
                                                               const ManifoldVertex &v1,
...
MTS_VARIANT void SpecularManifoldSingleScatter_AG<Float, Spectrum>::print_statistics() {
...
MTS_INSTANTIATE_CLASS(SpecularManifoldSingleScatter_AG)
