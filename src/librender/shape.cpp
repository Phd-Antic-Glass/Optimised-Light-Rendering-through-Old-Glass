...
MTS_VARIANT Shape<Float, Spectrum>::Shape(const Properties &props) : m_id(props.id()) {
    ...
    m_double_refraction     = props.bool_("double_refraction_caster", false);
}
...
MTS_VARIANT std::string Shape<Float, Spectrum>::id() const { return m_id; }

MTS_VARIANT typename Shape<Float, Spectrum>::PositionSample3f
Shape<Float, Spectrum>::sample_position(Float /*time*/, const Point2f & /*sample*/, Mask /*active*/) const {
...
#if defined(MTS_ENABLE_EMBREE)
template <typename Float, typename Spectrum> void embree_bbox(const struct RTCBoundsFunctionArguments *args) {
...
void embree_intersect_scalar(int *valid, void *geometryUserPtr, unsigned int geomID, RTCRay *rtc_ray, RTCHit *rtc_hit) {
    MTS_IMPORT_TYPES(Shape)
...
template <typename Float, typename Spectrum>
void embree_intersect_packet(int *valid, void *geometryUserPtr, unsigned int geomID, RTCRayW *rays, RTCHitW *hits) {
    MTS_IMPORT_TYPES(Shape)
...
template <typename Float, typename Spectrum> void embree_intersect(const RTCIntersectFunctionNArguments *args) {
    if constexpr (!is_array_v<Float>) {
        RTCRayHit *rh = (RTCRayHit *) args->rayhit;
        embree_intersect_scalar<Float, Spectrum>(args->valid, args->geometryUserPtr, args->geomID, (RTCRay *) &rh->ray,
                                                 (RTCHit *) &rh->hit);
    } else {
        RTCRayHitW *rh = (RTCRayHitW *) args->rayhit;
        embree_intersect_packet<Float, Spectrum>(args->valid, args->geometryUserPtr, args->geomID, (RTCRayW *) &rh->ray,
                                                 (RTCHitW *) &rh->hit);
    }
}

template <typename Float, typename Spectrum> void embree_occluded(const RTCOccludedFunctionNArguments *args) {
    if constexpr (!is_array_v<Float>) {
        embree_intersect_scalar<Float, Spectrum>(args->valid, args->geometryUserPtr, args->geomID, (RTCRay *) args->ray, nullptr);
    } else {
        embree_intersect_packet<Float, Spectrum>(args->valid, args->geometryUserPtr, args->geomID, (RTCRayW *) args->ray, nullptr);
    }
}
...
MTS_VARIANT void Shape<Float, Spectrum>::optix_prepare_geometry() { NotImplementedError("optix_prepare_geometry"); }
...
Shape<Float, Spectrum>::sample_direction(const Interaction3f &it, const Point2f &sample, Mask active) const {
...
MTS_VARIANT Float Shape<Float, Spectrum>::pdf_direction(const Interaction3f & /*it*/, const DirectionSample3f &ds, Mask active) const {
    MTS_MASK_ARGUMENT(active);

    Float pdf = pdf_position(ds, active), dp = abs_dot(ds.d, ds.n);
...
Shape<Float, Spectrum>::ray_intersect(const Ray3f & /*ray*/, Float * /*cache*/, Mask /*active*/) const {
    NotImplementedError("ray_intersect");
}

MTS_VARIANT typename Shape<Float, Spectrum>::Mask Shape<Float, Spectrum>::ray_test(const Ray3f &ray, Mask active) const {
    MTS_MASK_ARGUMENT(active);
...
MTS_VARIANT void Shape<Float, Spectrum>::fill_surface_interaction(const Ray3f & /*ray*/, const Float * /*cache*/,
                                                                  SurfaceInteraction3f & /*si*/, Mask /*active*/) const {
    NotImplementedError("fill_surface_interaction");
}

MTS_VARIANT typename Shape<Float, Spectrum>::SurfaceInteraction3f Shape<Float, Spectrum>::ray_intersect(const Ray3f &ray,
                                                                                                        Mask active) const {
    MTS_MASK_ARGUMENT(active);
...
Shape<Float, Spectrum>::normal_derivative(const SurfaceInteraction3f & /*si*/, bool /*shading_frame*/, Mask /*active*/) const {
    NotImplementedError("normal_derivative");
}

MTS_VARIANT typename Shape<Float, Spectrum>::UnpolarizedSpectrum
Shape<Float, Spectrum>::eval_attribute(const std::string & /*name*/, const SurfaceInteraction3f & /*si*/, Mask /*active*/) const {
    NotImplementedError("eval_attribute");
}

MTS_VARIANT Float Shape<Float, Spectrum>::eval_attribute_1(const std::string & /*name*/, const SurfaceInteraction3f & /*si*/,
                                                           Mask /*active*/) const {
    NotImplementedError("eval_attribute_1");
}

MTS_VARIANT int Shape<Float, Spectrum>::get_pair_id(Mask /*active*/) const { NotImplementedError("get_pair_id"); }

MTS_VARIANT int Shape<Float, Spectrum>::get_self_id(Mask /*active*/) const { NotImplementedError("get_self_id"); }

MTS_VARIANT typename Shape<Float, Spectrum>::Color3f
Shape<Float, Spectrum>::eval_attribute_3(const std::string & /*name*/, const SurfaceInteraction3f & /*si*/, Mask /*active*/) const {
    NotImplementedError("eval_attribute_3");
}

MTS_VARIANT Float Shape<Float, Spectrum>::eval_texture(int index, Point2f p, Mask active) const { NotImplementedError("eval_texture"); }
MTS_VARIANT Array<Float, 6> Shape<Float, Spectrum>::eval_curvature(Point2f p, Mask active,  bool full) const { NotImplementedError("eval_curvature"); }


MTS_VARIANT typename Shape<Float, Spectrum>::SurfaceInteraction3f Shape<Float, Spectrum>::eval_surfaceInteraction_from_uv(Point2f p, Mask active, SurfaceInteraction3f &si_) const {
    NotImplementedError("eval_surfaceInteraction_from_uv");
}

MTS_VARIANT typename Shape<Float, Spectrum>::ScalarFloat Shape<Float, Spectrum>::surface_area() const {
    NotImplementedError("surface_area");
}

MTS_VARIANT typename Shape<Float, Spectrum>::ScalarBoundingBox3f Shape<Float, Spectrum>::bbox(ScalarIndex) const { return bbox(); }

MTS_VARIANT typename Shape<Float, Spectrum>::ScalarBoundingBox3f Shape<Float, Spectrum>::bbox(ScalarIndex index,
...
MTS_VARIANT typename Shape<Float, Spectrum>::ScalarSize Shape<Float, Spectrum>::primitive_count() const { return 1; }

MTS_VARIANT typename Shape<Float, Spectrum>::ScalarSize Shape<Float, Spectrum>::effective_primitive_count() const {
...
MTS_VARIANT std::string Shape<Float, Spectrum>::get_children_string() const {
    ...
    if (m_emitter)
        children.push_back({ "emitter", m_emitter });
    if (m_sensor)
        children.push_back({ "sensor", m_sensor });
    if (m_interior_medium)
        children.push_back({ "interior_medium", m_interior_medium });
    if (m_exterior_medium)
        children.push_back({ "exterior_medium", m_exterior_medium });
