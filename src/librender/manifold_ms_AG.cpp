...
MTS_VARIANT std::atomic<int> SpecularManifoldMultiScatter_AG<Float, Spectrum>::stats_solver_failed(0);
MTS_VARIANT std::atomic<int> SpecularManifoldMultiScatter_AG<Float, Spectrum>::stats_solver_succeeded(0);
MTS_VARIANT std::atomic<int> SpecularManifoldMultiScatter_AG<Float, Spectrum>::stats_bernoulli_trial_calls(0);
MTS_VARIANT std::atomic<int> SpecularManifoldMultiScatter_AG<Float, Spectrum>::stats_bernoulli_trial_iterations(0);
MTS_VARIANT std::atomic<int> SpecularManifoldMultiScatter_AG<Float, Spectrum>::stats_bernoulli_trial_iterations_max(0);

MTS_VARIANT void
SpecularManifoldMultiScatter_AG<Float, Spectrum>::init(const Scene *scene,
...
SpecularManifoldMultiScatter_AG<Float, Spectrum>::specular_manifold_sampling(const SurfaceInteraction3f &si,
                                                                          ref<Sampler> sampler) {
    ...
    for (size_t shape_idx = 0; shape_idx < shapes.size(); ++shape_idx) {
        ...
        if (!m_config.biased) {
            ...
            Spectrum specular_val = evaluate_path_contribution(si, ei,sampler);

            // Account for BSDF at shading point
            BSDFContext ctx;
            Spectrum bsdf_val = si.bsdf()->eval(ctx, si, si.to_local(direction));
            // std::cout << si.to_local(direction) << std::endl;
            ...
            // Contribution
            // bsdf_val=Spectrum(1.0f);
            value =  bsdf_val; // ICI
            value *= specular_val;
            value *= inv_prob_estimate; 
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::sample_path(const ShapePtr shape,
    ...
    Mask success_solve = newton_solver(si, ei,sampler);
    if (!success_solve) {
        return false;
    }
...
MTS_VARIANT Spectrum
SpecularManifoldMultiScatter_AG<Float, Spectrum>::evaluate_path_contribution(const SurfaceInteraction3f &si,
                                                                          const EmitterInteraction &ei_,Sampler *sampler) {
    ...
    ManifoldVertex vx(si,false, 0.f);
    vx.make_orthonormal();

    Spectrum path_throughput(1.f);
    Spectrum spec = specular_reflectance(si, ei);
    // std::cout << spec << std::endl;
    path_throughput *= spec;
    path_throughput *= geometric_term(vx, vy); // ICI
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::sample_seed_path(const ShapePtr shape,
    ...
    while (true) {
        ...
        ManifoldVertex vertex = ManifoldVertex(si, false, 0.f);
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::newton_solver(const SurfaceInteraction3f &si,
    ...
    while (iterations < m_config.max_iterations) {
        ...
        bool project_success = reproject(si,sampler);
        if (!project_success) {
            beta = 0.5f*beta;
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::compute_step_halfvector(const Point3f &x0,
                                                                       const EmitterInteraction &ei) {
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::compute_step_anglediff(const Point3f &x0,
                                                                      const EmitterInteraction &ei) {
    ScopedPhase scope_phase(ProfilerPhase::SMS_computeDirection);
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::reproject(const SurfaceInteraction3f &si_,Sampler *sampler) {
...
MTS_VARIANT Spectrum
SpecularManifoldMultiScatter_AG<Float, Spectrum>::specular_reflectance(const SurfaceInteraction3f &si_,
    ...
    Spectrum bsdf_val(1.f);
        // std::cout<<"path in specular_reflectance"<<std::endl;
    for (size_t k = 0; k < m_current_path.size(); ++k) {
        si = m_scene->ray_intersect(ray);
        if (!si.is_valid()) {
            return 0.f;
        ...
        // std::cout<<m_current_path[k].to_string()<<std::endl;
    }
...
// HERE
MTS_VARIANT Float
SpecularManifoldMultiScatter_AG<Float, Spectrum>::geometric_term(const ManifoldVertex &vx,
    ...
    // std::cout << "Vertex path in Geometry_Factor : " << std::endl;
    // std::cout << "VX = " << vx.to_string() << std::endl;
    // std::cout << "v = " << std::endl;
    // for (size_t i = 0; i < k; ++i) {
...
MTS_VARIANT typename SpecularManifoldMultiScatter_AG<Float, Spectrum>::Mask
SpecularManifoldMultiScatter_AG<Float, Spectrum>::invert_tridiagonal_step(std::vector<ManifoldVertex> &v) {
...
// HERE
MTS_VARIANT Float 
SpecularManifoldMultiScatter_AG<Float, Spectrum>::invert_tridiagonal_geo(std::vector<ManifoldVertex> &v) {
...
MTS_VARIANT void SpecularManifoldMultiScatter_AG<Float, Spectrum>::print_statistics() {
...
MTS_INSTANTIATE_CLASS(SpecularManifoldMultiScatter_AG)
