...
template <typename Float, typename Spectrum, uint32_t Channels, bool Raw> class BitmapTextureImpl;

/// Bilinearly interpolated bitmap texture.
template <typename Float, typename Spectrum> class BitmapTexture final : public Texture<Float, Spectrum> {
public:
    ...
    BitmapTexture(const Properties &props) : Texture(props) {
        ...
        if (any(m_bitmap->size() < 2)) {
            ...
            ref<ReconstructionFilter> rfilter = PluginManager::instance()->create_object<ReconstructionFilter>(Properties("tent"));
    ...
    template <uint32_t Channels, bool Raw> using Impl = BitmapTextureImpl<Float, Spectrum, Channels, Raw>;
    ...
    std::vector<ref<Object>> expand() const override {
        ...
        switch (m_bitmap->channel_count()) {
            case 1:
                result = m_raw ? (Object *) new Impl<1, true>(props, m_bitmap, m_name, m_transform, m_mean)
                               : (Object *) new Impl<1, false>(props, m_bitmap, m_name, m_transform, m_mean);
                break;

            case 3:
                result = m_raw ? (Object *) new Impl<3, true>(props, m_bitmap, m_name, m_transform, m_mean)
                               : (Object *) new Impl<3, false>(props, m_bitmap, m_name, m_transform, m_mean);
                break;

            default:
                Throw("Unsupported channel count: %d (expected 1 or 3)", m_bitmap->channel_count());
        }
...
template <typename Float, typename Spectrum, uint32_t Channels, bool Raw> class BitmapTextureImpl final : public Texture<Float, Spectrum> {
public:
    MTS_IMPORT_TYPES(Texture)

    BitmapTextureImpl(const Properties &props, const Bitmap *bitmap, const std::string &name, const ScalarTransform3f &transform,
                      ScalarFloat mean)
        : Texture(props), m_resolution(bitmap->size()), m_name(name), m_transform(transform), m_mean(mean) {
        m_data = DynamicBuffer<Float>::copy(bitmap->data(), hprod(m_resolution) * Channels);

    }

    ScalarVector2i resolution() const override { 
        return ScalarVector2i(m_resolution.x(), m_resolution.y()); 
        }
    ...
    MTS_INLINE auto interpolate(const SurfaceInteraction3f &si, Mask active) const {
        ...
        Point2f w1 = uv - Point2f(pos), w0 = 1.f - w1;
        ...
        StorageType v00 = gather<StorageType>(m_data, index, active), v10 = gather<StorageType>(m_data, index + 1, active),
                    v01 = gather<StorageType>(m_data, index + width, active), v11 = gather<StorageType>(m_data, index + width + 1, active);
        ...
        if constexpr (is_spectral_v<Spectrum> && !Raw && Channels == 3) {
            ...
            StorageType v0 = fmadd(w0.x(), v00, w1.x() * v10), v1 = fmadd(w0.x(), v01, w1.x() * v11);
...
template <uint32_t Channels, bool Raw> constexpr const char *bitmap_class_name() {
    if constexpr (!Raw) {
...
Class *BitmapTextureImpl<Float, Spectrum, Channels, Raw>::m_class = new Class(detail::bitmap_class_name<Channels, Raw>(), "Texture",
